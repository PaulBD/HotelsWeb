...
namespace core.flights.services
{
    public class FlightService : IFlightService
    {
        ...
        {
            var request = "flights?flyFrom=" + flyFrom + "&to=" + flyTo + "&dateFrom=" + dateFrom + "&dateTo=" + dateTo;

            if (flightType == "round")
            {
                request += "&returnFrom=" + returnFrom + "&returnTo=" + returnTo;
                request += "&returndtimefrom=" + returnDepartureTimeFrom + "&returndtimeto=" + returnDepartureTimeTo + "&returnatimefrom=" + returnArrivalTimeFrom + "&returnatimeto=" + returnArrivalTimeTo;
            }

            request += "&typeFlight=" + flightType + "&oneforcity=0&one_per_date=0&passengers=" + passengerTotal + "&adults=" + adultTotal + "&children=" + childTotal + "&infants=" + infantTotal;
            ...
            request += "&atimeto=" + arrivalTimeTo + "&stopoverfrom=" + stopOverFrom + "&stopoverto=" + stopOverTo;
        ...
        public AirportLocationDto ReturnAirports(string term, string locale)
        {
            var request = "locations/?term=" + term + "&location_types=airport&locale=" + locale;

            var response = _client.GetAsync(request).Result;

            var result = response.Content.ReadAsStringAsync().Result;

            return JsonConvert.DeserializeObject<AirportLocationDto>(result);
        }
    }
}
